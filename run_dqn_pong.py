...
# dont want to update the target model gradient
...
for frame_idx in range(1, num_frames + 1):
    ...
    # input tensor = 84 pixels by 84 pixels
    ...
    if frame_idx % 10000 == 0 and len(replay_buffer) > replay_initial:
        ...
        frame_plot.append(frame_idx)
        loss_plot.append(np.mean(losses, 0)[1])
        reward_plot.append(np.mean(all_rewards[-10:], 0)[1])
...
plt.title("Loss change during the training")
plt.xlabel("Frames")
plt.ylabel("Loss")
plt.plot(frame_plot, loss_plot)
plt.savefig("Loss_record.png")

plt.title("Reward change during the training")
plt.xlabel("Frames")
plt.ylabel("Reward")
plt.plot(frame_plot, reward_plot)
